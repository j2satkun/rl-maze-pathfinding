# -*- coding: utf-8 -*-
"""reinforcement_mouse_learning_NORADRENALINE.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1AuWhCNXxbUN9JREvWSCyYzggljrIro03
...
class NoradrenalineSystem:
    ...
    def update(self, stress_signal):
        # Instead of reward prediction error, let's say noradrenaline responds to uncertainty or stress
        if not isinstance(stress_signal, torch.Tensor):
            stress_signal = torch.tensor(stress_signal, dtype=torch.float32)

        self.phasic = torch.sigmoid(stress_signal)
...
class BioModel(nn.Module):
    ...
    def forward(self, x):
        ...
        modulated = torch.sigmoid(self.dopamine_modulation(x)) * self.noradrenaline
    ...
    def update_noradrenaline(self, signal):
        self.noradrenaline = self.noradrenaline_system.update(signal)
...
class BioExperience:
    ...
    def remember(self, trial):
        ...
        if len(self.memory) > 1:
            ...
            self.model.update_noradrenaline(stress_signal)
        ...
        with open(log_file, mode='w', newline='') as f:
            ...
            with open(f"{data_folder}/{model_name}_neuromodulators.csv", 'w', newline='') as nf:
                neurowriter = csv.DictWriter(nf, fieldnames=[
                    "epoch", "episode", "step", "noradrenaline", "reward", "status"
                ])
                ...
                for epoch in range(number_epoch):
                    ...
                    for episode in range(data_size):
                        ...
                        while not trial_over:
                            ...
                            # Dopamine-modulated exploration
                            if np.random.rand() < epsilon * (1 - self.model.noradrenaline.item()):
                            ...
                            if track_neuromod:
                                neurowriter.writerow({
                                    ...
                                    "noradrenaline": self.model.noradrenaline.item(),
                                    "reward": reward,
                                    "status": status
                                })
                        ...
                        for name, param in self.model.named_parameters():
                            if 'direct_pathway' in name and param.grad is not None:
                                param.grad *= (1 + self.model.noradrenaline.item())
