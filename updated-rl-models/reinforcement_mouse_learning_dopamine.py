# -*- coding: utf-8 -*-
"""reinforcement_mouse_learning_DOPAMINE.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1wP8wgql8eiZNt9ll7RlwnEI2VDNoZfB0
...
class BioModel(nn.Module):
    def __init__(self, maze_size, num_actions):
        ...
        self.register_buffer('dopamine', torch.tensor(0.5))  # Baseline level
    ...
    def forward(self, x):
        ...
        modulated = torch.sigmoid(self.dopamine_modulation(x)) * self.dopamine
    ...
    def update_dopamine(self, rpe):
        self.dopamine = self.dopamine_system.update(rpe)
...
class BioExperience:
    ...
    def remember(self, trial):
        ...
        if len(self.memory) > 1:
            ...
            rpe = avg_reward - self.model.dopamine.item()
            self.model.update_dopamine(rpe)
    ...
    def data(self, data_size=10):
        ...
        for idx, jdx in enumerate(np.random.choice(range(mem_size), data_size, replace=False)):
            ...
            # Calculate RPE
            if trial_over:
                rpe = reward - current_q[action]
            else:
                env_state_next_tensor = torch.FloatTensor(env_state_next).view(1, -1)
                next_q = self.model(env_state_next_tensor).squeeze(0).detach()
                rpe = reward + self.discount * torch.max(next_q) - current_q[action]

            # Update dopamine and targets
            self.model.update_dopamine(rpe)
            targets[idx] = current_q.clone()
            targets[idx, action] = current_q[action] + rpe
...
class BioExperiment:
    ...
    def train(self, log_file=None, **opt):
                ...
                for epoch in range(number_epoch):
                    ...
                    for episode in range(data_size):
                        ...
                        while not trial_over:
                            ...
                            if np.random.rand() < epsilon * (1 - self.model.dopamine.item()):
                            ...
                            if track_neuromod:
                                neurowriter.writerow({
                                    ...
                                    "dopamine": self.model.dopamine.item(),
                                    "reward": reward,
                                    "status": status
                                })
                        ...
                        for name, param in self.model.named_parameters():
                            if 'direct_pathway' in name and param.grad is not None:
                                param.grad *= (1 + self.model.dopamine.item())
        ...
        torch.save(self.model.state_dict(), f"{data_folder}/{model_name}.pth")
