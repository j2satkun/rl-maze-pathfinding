# -*- coding: utf-8 -*-
"""reinforcement_mouse_learning_ACETYLCHOLINE.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/18cUpwSvcIzyvOYjBmKIDCgfsDjKM3ZML
...
class AcetylcholineSystem:
    def __init__(self, baseline=0.3, tau=0.05):
        self.baseline = baseline
        self.tau = tau
        self.tonic = torch.tensor(baseline)
        self.phasic = torch.tensor(0.0)

    def update(self, rpe_signal):
        # rpe_signal = reward prediction error (positive or negative)
        if not isinstance(rpe_signal, torch.Tensor):
            rpe_signal = torch.tensor(rpe_signal, dtype=torch.float32)

        self.phasic = torch.tanh(rpe_signal)  # RPE modulation
...
class BioModel(nn.Module):
    def __init__(self, maze_size, num_actions):
        ...
        # Dopamine modulation layer
        self.dopamine_modulation = nn.Linear(64, 64)

        # Define both pathways explicitly
        ...
        self.register_buffer('acetylcholine', torch.tensor(0.3))
    ...
    def forward(self, x):
        ...
        modulated = torch.sigmoid(self.dopamine_modulation(x)) * self.acetylcholine

        # Process through both pathways
        direct = self.direct_pathway(modulated)
        indirect = self.indirect_pathway(modulated)

        # Combine pathways (D1 - D2) to get final action values
        q_values = direct - indirect

        return q_values.squeeze(0) if q_values.size(0) == 1 else q_values

    def update_acetylcholine(self, rpe_signal):
        self.acetylcholine = self.acetylcholine_system.update(rpe_signal)
...
class BioExperience:
    ...
    def remember(self, trial):
        ...
        # Update dopamine based on recent rewards
        if len(self.memory) > 1:
            recent_rewards = [t[2] for t in self.memory[-5:]]
            avg_reward = sum(recent_rewards) / len(recent_rewards)
            stress_signal = 1.0 - avg_reward  # example proxy: low reward -> high stress
            self.model.update_acetylcholine(stress_signal)
...
class BioExperiment:
    ...
    def train(self, log_file=None, **opt):
        ...
        data_size = opt.get('data_size', 32)  # Used as number of episodes per epoch
        model_name = opt.get('name', 'bio_model')
        ...
        with open(log_file, mode='w', newline='') as f:
            ...
            with open(f"{data_folder}/{model_name}_neuromodulators.csv", 'w', newline='') as nf:
                neurowriter = csv.DictWriter(nf, fieldnames=[
                    "epoch", "episode", "step", "dopamine", "reward", "status"
                ])
                ...
                for epoch in range(number_epoch):
                    ...
                    for episode in range(data_size):
                        ...
                        while not trial_over:
                            ...
                            # Acetylcholine-modulated exploration
                            if np.random.rand() < epsilon * (1 - self.model.acetylcholine.item()):
                            ...
                            if track_neuromod:
                                neurowriter.writerow({
                                    ...
                                    "acetylcholine": self.model.acetylcholine.item(),
                                    "reward": reward,
                                    "status": status
                                })
                        ...
                        for name, param in self.model.named_parameters():
                            if 'direct_pathway' in name and param.grad is not None:
                                param.grad *= (1 + self.model.acetylcholine.item())
